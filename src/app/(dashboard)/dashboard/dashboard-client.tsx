...
// Lazily load the recharts-based chart so its ~150KB stays out of the
// dashboard's initial bundle (and off the critical path on first paint).
const MonthlyChart = dynamic(
  () => import("~/components/charts/monthly-chart").then((m) => m.MonthlyChart),
  {
    ssr: false,
    loading: () => (
      <div className="h-[300px] animate-pulse rounded bg-muted/40" />
    ),
  },
);
